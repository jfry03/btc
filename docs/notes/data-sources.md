...
| **CryptoHFTData** | raw L2 diffs (no snapshots), one parquet per hour; L1 by replay from empty book | **2025-06-28 → now** | diff events every ~100 ms (mid-2025) → ~52 ms (Oct 2025) → ~26 ms (2026), ~43 levels each, id chain unbroken | free anonymous download, 60 req/min, `curl -L` | yes — two hourly files inspected |
